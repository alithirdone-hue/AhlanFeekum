...
namespace AhlanFeekum.UserProfiles
{
    public class UserProfileManager : UserProfileManagerBase
    {
        ...
        public UserProfileManager(IUserProfileRepository userProfileRepository, IIdentityUserRepository identityUserRepository, 
                        IdentityUserManager identityUserManager, IDataFilter dataFilter, IBlobContainer<UserProfileContainer> userProfileContainer)
        ...
        public virtual async Task<MobileResponse> CreatWithDetialsAsync(string name, bool isSuperHost, string password, IFormFile profilePhoto, string roleName, string? email = null, string? phoneNumber = null, string? latitude = null, string? longitude = null, string? address = null)
        {
            ...
            try
            {
                ...
                var existingUser = users.FirstOrDefault(u => u.Email == email || u.UserName == email || u.PhoneNumber == phoneNumber);
                if (existingUser != null)
                {
                    mobileResponse.Code = 501;
                    mobileResponse.Message = "User already exists.";
                    mobileResponse.Data = null;
                    return mobileResponse;
                }

                var identityUser = new Volo.Abp.Identity.IdentityUser(Guid.NewGuid(), email, email);

...
* Check the `ConnectionStrings` in `appsettings.json` files under the `AhlanFeekum.Blazor` and `AhlanFeekum.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p d6af8d31-b318-4eb3-97ef-84624926d5da
```

> `d6af8d31-b318-4eb3-97ef-84624926d5da` is the password of the certificate, you can change it to any password you want.
...
Run `AhlanFeekum.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `AhlanFeekum.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `AhlanFeekum.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
